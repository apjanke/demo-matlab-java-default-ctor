...
reporoot = demodfltctor.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building DemoMatlabJavaDefaultCtor...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(demodfltctor.internal.readtext(cfgFile));
...
fprintf('DemoMatlabJavaDefaultCtor built.\n')
