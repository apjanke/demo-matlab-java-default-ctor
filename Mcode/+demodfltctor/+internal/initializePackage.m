...
% that's done in demodfltctor.Settings.discover. It has to be that way so the package
% settings state can handle a `clear classes` gracefully.

% Do not re-initialize if already initialized
if demodfltctor.internal.util.getpackageappdata('initialized')
  return
end

% Don't depend on globals, to avoid circular dependency
%distroot = demodfltctor.globals.distroot;
...
demodfltctor.internal.util.getpackageappdata('initialized');
