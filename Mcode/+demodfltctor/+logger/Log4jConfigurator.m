classdef Log4jConfigurator
    ...
    % Examples:
    %
    % demodfltctor.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % demodfltctor.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % demodfltctor.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % demodfltctor.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % demodfltctor.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % demodfltctor.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % demodfltctor.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = demodfltctor.logger.Log4jConfigurator.getLog4jLevel(levelName);
